...
export default function Color() {
  ...
  // SAVE된 색상을 클릭하였을때 발생하는 이벤트
  const main_color_ch = (target) => {
    let backgroundColor = window.getComputedStyle(target).backgroundColor;
    // RGBA 값을 추출합니다.
    let rgbArray = backgroundColor.match(/\((.*?)\)/)[1].split(",");
    // 불투명토가 1인경우 값이 안들어와서 강제로 세팅
    if( null == rgbArray[3] ){
        rgbArray[3] = "1"; 
    }
    // 메인 컬러 박스의 색상을 변경
    ...
    // 색상 슬라이드의 RGBA값을 변경함
    setRgb_R(rgbArray[0].trim());
    setRgb_G(rgbArray[1].trim());
    setRgb_B(rgbArray[2].trim());
    setRgb_A(rgbArray[3].trim());

    // 색상 텍스트의 RGBA값을 변경함
    document.querySelectorAll(".range-slider__value").forEach(function(item, index){ 
      document.querySelectorAll(".range-slider__value")[index].textContent = rgbArray[index];
    });
  }
    ...
    <div className='main'>
      ...
      <div id='colorPalette' className='color_palette'>
        ...
        {colors.map((colorObj, index) => (
            <div key={index} id={`color_${colorObj.id}`} style={{backgroundColor: colorObj.color}}  onClick={(e) => {main_color_ch(e.target)}} className="save_color">
            {/* color content */}
            </div>
        ))}
        </div>
